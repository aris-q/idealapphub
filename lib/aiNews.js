...
export async function getIndustryInsights() {
  ...
  return [...startups, ...enterprise].sort((a, b) => b.ts - a.ts).slice(0, 20);
}
...
export async function getVCInvestorNews() {
  ...
  return [...sequoia, ...tcVc].sort((a, b) => b.ts - a.ts).slice(0, 20);
}
...
export async function getMarketViews() {
  ...
  const seen = new Set();
  const merged = [];
  for (const item of perTicker.flat().sort((a, b) => b.ts - a.ts)) {
