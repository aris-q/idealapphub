...
// Minimal RSS 2.0 <item> parser shared by the fetchers below.
function parseRssItems(xml, source, idPrefix) {
  ...
  return items
    .map((item, i) => {
      ...
      return {
        id: `${idPrefix}-${i}-${pubDate ? Date.parse(pubDate) : i}`,
        title: decodeXml(title).trim(),
        link: decodeXml(link).trim(),
        body: description ? decodeXml(description).trim() : undefined,
        source,
...
async function fetchRss(url, source, idPrefix) {
  try {
    const res = await fetch(url, { ...REVALIDATE, headers: { "User-Agent": "Mozilla/5.0" } });
    if (!res.ok) throw new Error(`${source} RSS ${res.status}`);
    return parseRssItems(await res.text(), source, idPrefix);
  } catch (err) {
    console.error(`[AINEWS] ${source} fetch error:`, err.message);
    return [];
  }
}

// Company-side signals (funding, pivots, hiring, product launches) via
// TechCrunch's startups and enterprise category feeds.
export async function getIndustryInsights() {
  const [startups, enterprise] = await Promise.all([
    fetchRss("https://techcrunch.com/category/startups/feed/", "TechCrunch", "tc-startups"),
    fetchRss("https://techcrunch.com/category/enterprise/feed/", "TechCrunch", "tc-enterprise"),
  ]);
  const merged = dedupeByLink([...startups, ...enterprise].sort((a, b) => b.ts - a.ts));
  return merged.slice(0, 20);
}

// What VCs are funding and saying: Sequoia's own "who we're backing" posts,
// plus TechCrunch's venture-capital tag for broader funding-round coverage.
export async function getVCInvestorNews() {
  const [sequoia, tcVc] = await Promise.all([
    fetchRss("https://www.sequoiacap.com/feed/", "Sequoia Capital", "seq"),
    fetchRss("https://techcrunch.com/tag/venture-capital/feed/", "TechCrunch", "tc-vc"),
  ]);
  const merged = dedupeByLink([...sequoia, ...tcVc].sort((a, b) => b.ts - a.ts));
  return merged.slice(0, 20);
}

// Market commentary on a curated set of AI-adjacent large-cap tickers,
// via Yahoo Finance's per-symbol headline RSS (free, no key).
const MARKET_TICKERS = ["NVDA", "MSFT", "GOOGL", "AMZN", "META", "TSLA", "AAPL", "AMD"];

export async function getMarketViews() {
  const perTicker = await Promise.all(
    MARKET_TICKERS.map((sym) =>
      fetchRss(
        `https://feeds.finance.yahoo.com/rss/2.0/headline?s=${sym}&region=US&lang=en-US`,
        "Yahoo Finance",
        `yf-${sym}`
      ).then((items) => items.map((item) => ({ ...item, ticker: sym })))
    )
  );
  const merged = dedupeByLink(perTicker.flat().sort((a, b) => b.ts - a.ts));
  return merged.slice(0, 20);
}
