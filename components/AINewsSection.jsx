...
function NewsRow({ item }) {
  return (
    <a href={item.link || "#"} target={item.link ? "_blank" : undefined} rel="noopener noreferrer" style={{
      ...
      <span style={{ fontSize: 12, color: "var(--text-primary)", lineHeight: 1.5, flex: "1 1 200px", minWidth: 0 }}>{item.title}</span>
    </a>
  );
}
...
export default function AINewsSection({ highlights = [], today = [], week = [] }) {
  return (
    <section id="news" style={{ padding: "40px clamp(16px, 5vw, 32px)", borderBottom: "1px solid var(--border)" }}>
      ...
      {highlights.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          <p style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, letterSpacing: 3, color: "var(--accent)", marginBottom: 12 }}>
            ...
            {highlights.map((item) => (
              <a key={item.id} href={item.link || "#"} target={item.link ? "_blank" : undefined} rel="noopener noreferrer" style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: "var(--accent)" }}>{formatDate(item.ts)}</span>
                  <SourceBadge source={item.source} />
                </div>
                <h3 style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: 6, lineHeight: 1.4 }}>{item.title}</h3>
                {item.body && <p style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.6 }}>{item.body}</p>}
              </a>
            ))}
          </div>
        </div>
