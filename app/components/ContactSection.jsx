...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatus("sending");
    // TODO: wire to email service later
    setTimeout(() => {
      setStatus("sent");
  ...
  const inputStyle = {
    ...
    fontSize: 12, fontFamily: "'Courier New', monospace", marginBottom: 10, outline: "none"
  };
    ...
    <section id="contact" style={{ padding: "40px 32px", borderBottom: "1px solid var(--border)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <textarea style={{ ...inputStyle, height: 80, resize: "none" }} placeholder="Your message..."
            value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required />

          <button type="submit" disabled={status === "sending"} style={{
            width: "100%", padding: "10px 24px", background: "var(--accent)",
